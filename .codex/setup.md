...
4. 변경 전 필요한 경우 작업자별 `docs/work-plans-<owner>/`에 작업 계획을 남긴다.
...
- 산출물은 한국어로 작성한다.
- 작업자별 산출물은 `docs/<artifact-type>-<owner>/` 형식을 사용한다. `<owner>`는 작업을 수행하는 사람 또는 에이전트 소유자를 식별하는 짧은 영문 소문자 이름이다.
- 현재 작업자의 `<owner>`를 기존 폴더명, 사용자 요청, 로컬 지시문에서 확인한다. 확인할 수 없으면 공용 폴더를 만들지 말고 사용자에게 물어본다.
